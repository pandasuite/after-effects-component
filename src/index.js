...
PandaBridge.init(() => {
  ...
  function myInit() {
    const jsonUrl = PandaBridge.resolvePath('animationFolder', './') + properties.animationPath;
    // eslint-disable-next-line no-nested-ternary
    const loop = properties.loop === -1
      ? true
      : (properties.loop === 0 ? false : properties.loop + 1);
  ...
  PandaBridge.listen('forward', (args) => {
    ...
    setFrame(Math.min(animation.currentRawFrame + parseInt(props.frame),
  ...
  PandaBridge.listen('increaseSpeed', (args) => {
    ...
    if (animation) {
      const speed = Math.min(currentSpeed + parseFloat(props.speed), properties.maxSpeed);
  ...
  PandaBridge.listen('decreaseSpeed', (args) => {
    ...
    if (animation) {
      const speed = Math.max(currentSpeed - parseFloat(props.speed), properties.minSpeed);
      animation.setSpeed(speed);
      currentSpeed = speed;
    }
  });
  ...
  PandaBridge.synchronize('speed', (percent) => {
    if (animation) {
      animation.setSpeed(
        ((properties.maxSpeed - properties.minSpeed) * percent / 100) + properties.minSpeed,
      );
    }
  });
  ...
  function onTimeUpdate() {
    ...
    if (!fromSynchro) {
      let percent = parseInt(animation.currentRawFrame) * 100 / (animation.totalFrames - 1);
      if (properties.loop !== -1) {
        percent = (parseInt(animation.currentRawFrame)
        + ((properties.loop - nbLoop) * animation.totalFrames)) * 100
